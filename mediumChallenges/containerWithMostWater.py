...
def find_greatest_volume(height):
    return_value = 0
    left_index = 0
    ...
    while left_index < right_index:
        ...
        return_value = max((width*min_height), return_value)
        nxt_right_index = right_index - 1
        nxt_left_index = left_index + 1
        if height[left_index] > height[right_index]:
            right_index -= 1
        elif height[left_index] < height[right_index]:
            left_index += 1
        else:
            if height[nxt_right_index] > height[nxt_left_index]:
                right_index -= 1
            else:
                left_index += 1
...
#     return_value = 0
#     left_index = 0
#     right_index = len(int_list)-1
#     while left_index < right_index:
#         width = right_index - left_index
#         height = min(int_list[left_index], int_list[right_index])
#         return_value = max((width*height), return_value)
#         nxt_right_index = right_index - 1
#         nxt_left_index = left_index + 1
#         if int_list[left_index] > int_list[right_index]:
#             right_index -= 1
#         elif int_list[left_index] < int_list[right_index]:
#             left_index += 1
#         else:
#             if int_list[nxt_right_index] > int_list[nxt_left_index]:
#                 right_index -= 1
#             else:
#                 left_index += 1
...
#         if len(int_list[index1:original_length]) <= 1:
#             continue
...
#             if index2 == 0 or len(int_list[index1:negative_index]) < 2:
#                 negative_index -= 1
#                 continue
#             else:
